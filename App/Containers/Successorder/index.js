...
const Successorder = (props) => {
  ...
  useEffect(() => {
    // Get async storage setOrder ke server BINA (ambil data odr_id dan rtl_id dan qty) setOrder
    ...
    AsyncStorage.getItem('setOrder').then(response => {
      ...
      console.log('SET ORDER -----------> ' + JSON.stringify(data));

      setState(state => ({
        ...state,
        odr_id: data.odr_id[0],
        odr_rtl_id: data.value.odr_rtl_id,
        odd_qty: data.value.odd_qty,
      }))
      console.log('odr id ' + JSON.stringify(state.odr_id));
      console.log('rtl id ' + JSON.stringify(state.odr_rtl_id));
      console.log('qty ' + JSON.stringify(state.odd_qty));
